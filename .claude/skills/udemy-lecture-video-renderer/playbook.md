...
| Letter-acronym aliasing (e.g. API) | Round-1 `<alias>ay pee eye</alias>` → "eye-pee-eye" (homophone). Round-3 `<alias>A. P. I.</alias>` → "A dot P dot I dot" (literal period-pauses). Round-4 no alias → "A pie" (slurred). Round-6 (lecture 2.2 round-3 of polish) tried verbose `<alias>letter A, letter P, letter I</alias>` after a perceived single-render regression — ElevenLabs applied it VERBATIM ("letter ay, letter pee, letter eye" on every mention). | **Round-5 / current working form: `<alias>A, P, I</alias>` (commas).** Commas create micro-pauses (~50ms) without the full-stop pauses periods cause, AND the letters stay distinct so the TTS can't slur them into a word. Pattern applies to other letter-acronyms (SDK, CLI, MCP, HTTP, URL, UUID) — try the comma form first if a homophone surfaces. **First-debug rule on regression: when API (or any letter-acronym) sounds wrong on a render, do NOT escalate the alias text. Aliases are read verbatim — verbose forms produce verbose audio. FIRST verify the PLS uploaded + the cached dict ID on ElevenLabs is current (`tts-config.json` `pls_sha256` matches the merged-PLS hash; the dict ID resolves on ElevenLabs and contains the expected rule). Stale-cache / failed-upload regressions look identical to "wrong alias" regressions but the fix is re-upload, not re-text.** |
| Plural-of-letter-acronym (e.g. APIs, SDKs, CLIs) | Naive `<alias>A, P, I, s</alias>` reads as "ay, pee, eye, (pause) ess" — the trailing standalone `s` gets vocalized as a separate letter with an awkward gap. | Use the word containing the natural plural ending: `<alias>A, P, eyes</alias>` for APIs (reads "ay, pee, eyes"), `<alias>ess dee kays</alias>` for SDKs, `<alias>see ell eyes</alias>` for CLIs, `<alias>you are ells</alias>` for URLs. Mirrors the SDK/CLI/URL plural forms already in `pronunciation.template.pls`. |
| Em-dash artifact in narration (e.g. `level — not`) | ElevenLabs sometimes vocalizes the em-dash as a brief "eh" / "e" sound in certain phonetic neighborhoods — particularly between a word ending on a hard consonant (`level`, `top`) and a glottal-onset word (`not`, `outside`). Symptom: viewer hears "...top level eh not inside messages" with an audible vowel where there should be just a pause. Observed on lecture 2.2 round-3 SLIDE 6. | Replace the em-dash with a comma at the specific spot: `at the top level, not inside messages`. Do NOT globally sweep em-dashes — most renders cleanly. The artifact is positional. Authoring guidance + heuristic in `udemy-lecture-writer/SKILL.md` Rule 8 ("Em-dashes in narration: use sparingly"). |
