...
| Alias with `ay` for letter A (e.g. `<alias>ay pee eye</alias>` for API) | "ay" is read as homophone "eye" → "API" sounds like "eye-pee-eye" | Use period-separated capital letters: `<alias>A. P. I.</alias>`. The period forces the TTS into letter-reading mode and avoids the `ay → eye` homophone collision. Same pattern for SDK / CLI / MCP / HTTP / URL / UUID. **Round-4 update:** the API alias itself was REMOVED entirely from `pronunciation.template.pls` — both `ay pee eye` and `A. P. I.` aliases sounded wrong in production. Base `eleven_multilingual_v2` TTS handles "API" natively. Treat other letter-acronyms case-by-case: keep the alias unless a render reveals a similar regression. |
